#code where i show each individual comparison image with their dtw_score
import PIL
import cv2
import pytesseract
import numpy as np
...
# Initialize a list to store similarity scores for each pair of corresponding bounding boxes
similarity_scores = []
...
for box1, box2 in zip(boxes1.splitlines(), boxes2.splitlines()):
    ...
    cv2.putText(comparison_image, f"DTW: {similarity_score:.2f}", (10, common_shape[0] + 30),
...
# Calculate the collective similarity score as the average of all individual scores
collective_similarity_score = np.mean(similarity_scores)

# Print the collective similarity score
print("Collective Similarity Score:", collective_similarity_score)
