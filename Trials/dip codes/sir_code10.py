...
def find_differences(img1, img2):
    ...
    # Compute the structural similarity index for the entire image
    ...
    # Mark differences in the image
    marked_img = cv2.cvtColor(img1, cv2.COLOR_GRAY2BGR)  # Convert to BGR format

    if len(contours):
        for c in contours:
            x, y, w, h = cv2.boundingRect(c)
            cv2.rectangle(marked_img, (x, y), (x + w, y + h), (0, 0, 255), 4)
            
    cv2.imshow("k", marked_img)
    cv2.waitKey(0)
