            ...
            if response.status_code == 200:
                ...
                with tempfile.NamedTemporaryFile(delete=False) as temp1:
                    ...
                    try:
                        ...
                        if (similarity_grade != 0) and (not np.isnan(similarity_grade)) and (similarity_grade is not None):
                            try:
                                canvas = marking(temp1_path, temp2_path)
                            except Exception as marking_error:
                                print("Error during marking:", str(marking_error))

                        textual_image2 = add_text(ocr_image2, similarity_grade,boxes2, height2)

                        try:
                            image2_base64, canvas_base4 = encode_images(textual_image2, canvas)
                        except Exception as encode_error:
                            print("Error during image encoding:", str(encode_error))
