# Calculating similarity using optical flow + assigning value 0.0 to unequal characters
import cv2
import pytesseract
import PIL.Image
...
def calculate_optical_flow(img1, img2):
    ...
    # Resize images to the same dimensions (adjust dimensions as needed)
...
def match(img1, img2):
    ...
    # Calculate similarity based on the average magnitude of optical flow
    similarity_value = "{:.2f}".format(np.mean(magnitude))
    ...
    cv2.putText(img1, 'Score: ' + str(similarity_value), 
