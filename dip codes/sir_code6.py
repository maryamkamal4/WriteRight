...
i = 0  # Index for characters in text1
j = 0  # Index for characters in text2

# Loop through bounding boxes and calculate similarity with space handling
for box1, box2 in zip(boxes1.splitlines(), boxes2.splitlines()):
    
    # Check if char1 or char2 is a space, and only increment the corresponding index
    if text1[i].isspace():
        i += 1
    if text2[j].isspace():
        j += 1

    # Get the current character from text1 and text2 after possible increments
    char1 = text1[i] if i < len(text1) else ''
    char2 = text2[j] if j < len(text2) else ''
